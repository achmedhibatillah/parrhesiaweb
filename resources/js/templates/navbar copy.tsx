                ...
                <Container className={'w-[99%]'}>
                    <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-7">
                        <div className="col-span-1 md:col-span-1 lg:col-span-1 xl:col-span-1 relative flex">
                            <div className="flex gap-2 px-4 py-3 items-center text-light bg-passion rounded-m border-5 border-light shadow-md">
                        ...
                        <div className="col-span-1 md:col-span-3 lg:col-span-4 xl:col-span-6 flex relative justify-end items-center">
                                            ...
                                            >
                                                {menu.map(item => (
                                                    <a
                                                        key={item.page}
                                                        href={item.url}
                                                        className="hidden lg:flex items-end gap-2 md:2-[180px] xl:w-[190px] h-[30px] bg-gradient-to-br from-passion to-energy hover:bg-gradient-to-tl hover:from-passion hover:to-passion transition-colors rounded-full relative"
                                                    >
                                                        <div className="bg-light inset-shadow-sm inset-shadow-indigo-300/80 h-[30px] w-[30px] -ms-1.5 rounded-full flex justify-center items-center">
                                                            <i className={`text-[14px] text-passion fas fa-${item.icon}`}></i>
                                                        </div>
                                                        <p className="m-0 md:text-[10px] lg:text-[14px] text-light font-bold mb-1">{item.title}</p>
                                                        <i className={`fas fa-circle absolute right-1 top-[7px] ${page === item.page ? 'text-green-400' : 'text-light'}`}></i>
                                                    </a>
                                                ))}
                                            </motion.div>
                                        </motion.div>
                            ...
                            <div className="ms-3 bg-amber-200">
                                <a href="login" className="bg-blue-700 hover:bg-blue-800 transition-colors text-light border-5 border-light flex flex-col items-center rounded-xl p-2 pt-3">
                                    <i className="fas fa-lock"></i>
                                    <p className="text-[10px]">Login</p>
                                </a>
                            </div>
                            <div className="flex justify-center items-center ms-3">
                                <div className={`hover:bg-amber-950 transition-colors ${open ? "shadow-md/20 bg-gradient-to-br from-passion to-energy" : "bg-passion"} text-light rounded-full flex justify-center items-center h-10 w-10 cursor-pointer lg:hidden`} onClick={toggleMenu}>
                    ...
                    <div id="menu-sm" className={`lg:hidden overflow-hidden transition-all duration-900 ease-in-out ${open ? 'opacity-100 max-h-[100vh]' : 'opacity-0 max-h-0'}`}>
                        <div className="mt-5">
                            {menu.map(item => (
                                <a key={item.page} href={item.url} className="flex items-end gap-2 w-[98%] h-[45px] bg-gradient-to-br from-passion to-energy hover:bg-gradient-to-tl hover:from-passion hover:to-passion transition-colors rounded-full relative mb-2 ms-1.5 shadow">
